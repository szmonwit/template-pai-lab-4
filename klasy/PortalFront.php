...
class PortalFront extends Portal
{
  ...
  function registerUser(){
    ...
    function showSearchForm() { 
      ...
      function showSearchResult() { // Określenie warunku dla autora
        if (isset($_GET['autor']) && $_GET['autor'] != '') {
           ...
           $cond1 = " AND a.`Nazwa` LIKE '%" . $autor . "%' "; } 
            ...
            if (isset($_GET['tytul']) && $_GET['tytul'] != '') { 
              ...
              $query = 'SELECT k.`Tytul`, GROUP_CONCAT(a.`Nazwa`) AS `Autor`, ' . 'k.`ISBN`, w.`Nazwa` AS `Wydawnictwo`, k.`Cena`, k.`Id` AS `Id` ' . 'FROM Ksiazki k JOIN Wydawnictwa w ON (k.WydawnictwoId = w.Id) ' . 'JOIN KsiazkiAutorzy ka ON (ka.`KsiazkaId` = k.`Id`) ' . 'JOIN Autorzy a ON (ka.`AutorId` = a.`Id`) WHERE 1=1 ' . $cond1 . $cond2 . 'GROUP BY k.`Id` ORDER BY `Autor`, `Tytul`, `Wydawnictwo`'; 
            ...
            function showBookDetails() { 
              ...
              if (!isset($_GET['id']) || ($id = intval($_GET['id'])) < 1) { 
                $komunikat = 'Brak książki o podanym identyfikatorze.'; } 
                  ...
                  $query = 'SELECT `Tytul`, `ISBN`, `Rok wydania` AS Rok, `Opis`, `Cena`,' . 'w.`Nazwa` AS `Wydawnictwo`, GROUP_CONCAT(a.`Nazwa`) AS Autor,' . 'k.`Id` AS `Id` FROM Ksiazki k ' . 'JOIN KsiazkiAutorzy ka ON (k.`Id`=ka.`KsiazkaId` AND k.id=' . $id . ')' . 'JOIN Autorzy a ON (a.`Id` = ka.`AutorId`) ' . 'JOIN Wydawnictwa w ON (w.`Id`=k.`WydawnictwoId`) GROUP BY k.`Id`'; 
